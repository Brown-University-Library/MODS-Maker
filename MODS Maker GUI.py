...
def processExceltoMODS(MyGUI, chosenfile, chosensheet, output_path):
    langissue = False
    csvdata = {}
    ...
    for column in requiredcolumns:
        if (column in columnsinsheet) == False:
            #MyGUI.outputtext.insert(END,"Missing spreadsheet column: " + column + '\n')
            missingcolumns.append(column)

    if len(missingcolumns) != 0:
        MyGUI.outputtext.insert(END,"*Missing Columns Detected*" + '\n')
        MyGUI.outputtext.insert(END,"The columns below are missing from your spreadsheet. The script will continue without them." + '\n\n')
        MyGUI.outputtext.see("end")
        for column in missingcolumns:
            MyGUI.outputtext.insert(END,"   " + column + '\n')
            MyGUI.outputtext.see("end")
        #messageToUser("*Missing Columns Detected*", "The columns are missing from your spreadsheet. The script will now continue without them.")
        originalfile = chosenfile
        #for column in missingcolumns:
        #    workbooktochange = load_workbook(chosenfile)
        #    sheettochange = workbooktochange[chosensheet]
        #    sheettochange.insert_cols(1,1)
        #    sheettochange.cell(row=1, column=1).value = column
        #    workbooktochange.save(os.getcwd() + "/cache/missingcolumnsedit.xlsx")

        #    chosenfile = os.getcwd() + "/cache/missingcolumnsedit.xlsx"
        #    excel = xlrd.open_workbook(chosenfile)
    ...
    for row in csvdata:
        ...
        with open(output_path+'/'+filename + ".mods", 'wb') as f:
            ...
            MyGUI.outputtext.insert(END, "Writing" + filename + ".mods" + "\n")
            MyGUI.outputtext.see("end")
...
class MyGUI:
    ...
    def __init__(self):
        ...
        self.__mainWindow.title('MODS Maker')
        self.frame1 = Frame(self.__mainWindow)
        self.frame1.config(width=1000)
        self.frame1.grid(row=1,column=1, padx=10, pady=10)
        self.buttonframe = Frame(self.frame1)
        self.buttonframe.grid(row=4,column=1, padx=10, pady=10)
        self.labelframe = LabelFrame(self.frame1, text="Excel file")
        self.labelframe.config(height=800)
        self.labelframe.config(width = 1000)
        self.labelframe.grid(row=2,column=1, padx=10, pady=10)
        self.outputframe = Frame(self.frame1)
        self.outputframe.grid(row=3,column=1, padx=10, pady=10)
        self.outputframelabel = LabelFrame(self.outputframe, text="Output directory")
        self.outputframelabel.grid(row=2,column=1, padx=10, pady=10)
        self.outputlabel = Label(self.outputframelabel, text=self.outputdirectory, font=("arial", 10),anchor="w")
        self.outputlabel.grid(row=4,column=1, padx=10, pady=0)


        self.listscrollbar = Scrollbar(self.__mainWindow)
        self.openbutton = Button(self.frame1, text=u"\u25CE Open File", command=self.openCallBack, font=("arial", 12))
        self.openbutton.grid(row=1,column=1, padx=10, pady=5)
        self.processbutton = Button(self.buttonframe, text=u"\u2661 Process File", command=self.processCallBack, font=("arial", 12))
        self.processbutton.grid(row=2,column=1, padx=10, pady=5)
        self.outputbutton = Button(self.buttonframe, text=u"\u2662 Change Output Directory", command=self.outputCallBack, font=("arial", 12))
        self.outputbutton.grid(row=3,column=1, padx=10, pady=5)

        self.filenamelabel = Label(self.labelframe, text = "                     ", font=("arial", 12), anchor="w")
        self.filenamelabel.grid(row = 2, column=1, padx=10, pady=10)
        self.sheetlabel = Label(self.labelframe, text = "Choose sheet:", font=("arial", 12),anchor="w")
        self.sheetlabel.grid(row = 3, column=1, padx=10)
        self.sheetlist = Listbox(self.labelframe, {'yscrollcommand':self.listscrollbar})
        self.sheetlist.config(width=45)
        self.listscrollbar.config(command=self.sheetlist.yview)
        self.sheetlist.grid(row = 4, column=1, padx=10, pady=10)


        self.outputtext = Text(self.__mainWindow)
        self.outputtext.grid(row = 1, column=2, padx=10, pady=10)

        mainloop()

    def openCallBack(self):
        self.chosenfile = tkFileDialog.askopenfilename(initialdir = os.getcwd(),title = "Select file",filetypes = (("Excel files","*.xlsx"),("all files","*.*")))
        self.filenamelabel['text'] = os.path.basename(self.chosenfile)
        excel = xlrd.open_workbook(self.chosenfile)
        sheetnames = excel.sheet_names()
        self.sheetlist.delete(0,'end')
        for index, sheet in enumerate(sheetnames):
            self.sheetlist.insert(index,sheet)
    def processCallBack(self):
        try:
            self.chosensheet = self.sheetlist.get(self.sheetlist.curselection())
        except:
            print("nope")
        if self.chosenfile != '' and self.chosensheet != '':
            processExceltoMODS(self,self.chosenfile, self.chosensheet, self.outputdirectory)
        else:
            tkMessageBox.showinfo("Alert", "To continue, please select an Excel file and a sheet to process.")
    def outputCallBack(self):
        self.outputdirectory = tkFileDialog.askdirectory(initialdir = os.getcwd(),title = "Select output directory")
